...
class Matrix_server():
    ...
    def load_config(self,filename = ""):
        if filename == "":
            filename=self.servername.strip('https://')+'.json'
        try:
            file = open(filename)
            self.auth_user = json.load(file)
            self.set_access_token(self.auth_user['access_token'])
            self.servername="https://"+self.auth_user['home_server']
            file.close()
            return True
        except:
            return None
    ...
    def get_room_id_from_alias(self, room_alias):
        url=self.servername+f"/_synapse/client/r0/directory/room/{urllib.parse.quote(room_alias)}"
    ...
    def get_users_from_room(self, room_id):
        url=self.servername+f"/_snyapse/client/r0/rooms/{urllib.parse.quote(room_id)}/joined_members"
        res = requests.get(url,headers=self.header)
        if res.status_code==200:
                return res.json()
        else:
    ...
    def get_rooms(self):
        url=self.servername+"/_synapse/admin/v1/rooms"
        res = requests.get(url,headers=self.header)
        if res.status_code==200:
    ...
    def get_users(self):
        url=self.servername+"/_synapse/admin/v2/users"
        res = requests.get(url,headers=self.header)
        if res.status_code==200:
    ...
    def delete_room(self, room_id):
        url=self.servername+f"/_synapse/admin/v1/rooms/{room_id}/delete"
    ...
    def new_user(self, user_data):
        url=self.servername+f"/_synapse/admin/v2/users/{urllib.parse.quote(user_data['user_id'])}"
    ...
    def deactivate_user(self, user_id):
        url=self.servername+f"/_synapse/admin/v1/deactivate/{urllib.parse.quote(user_id)}"
    ...
    def get_user(self, user_id):
        url=self.servername+f"/_synapse/admin/v2/users/{urllib.parse.quote(user_id)}"
        res = requests.get(url, headers=self.header)
        if res.status_code==200:
            return res.json()
        else:
    ...
    def reset_password(self,user_id,password, logout_devices = True):
        url=self.servername+f"/_synapse/admin/v1/reset_password/{urllib.parse.quote(user_id)}"
    ...
    def get_joined_rooms(self, user_id):
        url=self.servername+f"/_synapse/admin/v1/users/{urllib.parse.quote(user_id)}/joined_rooms"
        res = requests.get(url, headers=self.header)
        if res.status_code==200:
            return res.json()
        else:
    ...
    def get_user_devices(self,user_id):
        url=self.servername+f"/_synapse/admin/v2/users/{urllib.parse.quote(user_id)}/devices"
        res = requests.get(url, headers=self.header)
        if res.status_code==200:
            return res.json()
        else:
